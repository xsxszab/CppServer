...
class HttpParser : std::enable_shared_from_this<HttpParser> {
 public:
...
class HttpManager {
 ...
 private:
  void RegisterConn(std::shared_ptr<cppserver_core::Connection> conn);

  std::unordered_map<std::shared_ptr<cppserver_core::Connection>,
                     std::shared_ptr<HttpParser>>
      http_connections_;
};
